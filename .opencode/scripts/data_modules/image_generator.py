...
class ImageGenerator:
    ...
    async def _save_prompt_to_file(self, prompt: str, output_path: Path, label: str = "") -> Optional[Path]:
        """仅保存提示词到文件（替代 API 调用），返回文件路径"""
        prompt_dir = output_path.parent
        prompt_dir.mkdir(parents=True, exist_ok=True)
        # 将 .png 后缀替换为 .txt
        txt_path = prompt_dir / (output_path.stem + ".txt")
        content = f"# {label or output_path.stem} 提示词\n# 请将此提示词复制到 nano banana 或 GPT-Image-2 生成图片\n# 生成后保存为: {output_path.name}\n\n{prompt}\n"
        txt_path.write_text(content, encoding="utf-8")
        logger.info(f"[提示词已保存] {txt_path}")
        return txt_path
    ...
    async def generate(
        ...
        """生成图片并保存到本地；若未配置真实 API key 则输出提示词文件"""
        is_real_key = bool(self.config.image_api_key) and self.config.image_api_key not in (
            "your_modelscope_sdk_token", "your_api_key", "not-needed", "your_image_api_key",
        )
        if not is_real_key:
            return await self._save_prompt_to_file(prompt, output_path, label=label)
    ...
    async def generate_cover(
        ...
        return await self.generate(prompt, output_path, label="封面")
    ...
    async def generate_character(
        ...
        return await self.generate(prompt, output_path, label=f"角色-{name}")
