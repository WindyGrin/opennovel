...
def _is_localhost_url(url: str) -> bool:
    return url and ("localhost" in url or "127.0.0.1" in url)


def _ensure_local_server(config) -> None:
    """若配置指向 localhost 且服务未运行，自动后台启动并等待就绪"""
    import subprocess, time, sys
    urls_to_check = []
    if _is_localhost_url(config.embed_base_url):
        urls_to_check.append(config.embed_base_url.rstrip("/") + "/health")
    if _is_localhost_url(config.rerank_base_url):
        rurl = config.rerank_base_url.rstrip("/") + "/health"
        if rurl not in urls_to_check:
            urls_to_check.append(rurl)
    ...
    # 寻找 local_embed_serve.py
    import os
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(script_dir))  # .opencode/
    serve_script = os.path.join(project_root, "scripts", "local_embed_serve.py")
    if not os.path.exists(serve_script):
        return  # 没找到脚本，跳过

    logger.info("本地 AI 服务未启动，正在自动启动...")
    ...
    # 轮询等待就绪
    logger.info("等待本地 AI 服务就绪（首次需加载模型，约 30-60 秒）...")
    for _ in range(120):
        time.sleep(2)
        try:
            urllib.request.urlopen(urls_to_check[0], timeout=2)
            logger.info("本地 AI 服务已就绪")
            return
        except Exception:
            pass
    logger.warning("本地 AI 服务启动超时，将使用云端备选方案")
...
def get_client(config=None) -> ModalAPIClient:
    ...
    if _client is None or config is not None:
        ...
        _ensure_local_server(cfg)
        _client = ModalAPIClient(cfg)
