...
从 `设定集/崩点标签库.md` 加载已知崩点标签，对本章做风格扫描：

- **句式重复**：同一章内相同句式出现≥5次 → high，"句式复读机"标签
- **匠气引号**：已确立概念（如"磨""刚好"）在10章后仍加引号 → medium，"匠气太重"标签
- **谜语人参演**：主角说话留半句等人追问 + 每章必有感知展示 → high，"谜语人装逼"标签
- **理论灌输**：叙事跳出角色进行≥3句的世界观解释 → medium，"理论灌输式叙事"标签
- **旁白抢戏**：角色行为后旁白解释"为什么别人做不到" → medium，"旁白抢戏"标签
- **感知展览**：连续3章每章有"只有主角能感知到"的展示 → low，"感知展览"标签

severity 规则：句式重复≥5次→high；匠气引号→medium；谜语人化→high；理论灌输→medium

创建与崩点标签库的交叉引用：如果本章触发了已有标签，在报告中标注 `崩点标签: [标签名]（最近: 第X章）`
...
```json
{
  ...
  "dimension_scores": { "设定一致性":85, "连贯性":82, "人物塑造":78, "追读力":88, "爽点密度":90, "节奏控制":80, "文笔风格":85 },
  ...
  "metrics": {
    ...
    "sentence_repeats": 0, "quoted_metaphors": 0, "riddle_lines": 0, "info_dump_blocks": 0
  },
