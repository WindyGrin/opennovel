...
IMAGE_API_KEY=your_modelscope_sdk_token    ← 保留此占位符 = 提示词输出模式
IMAGE_SIZE=1:1
```
...
| 模式 | IMAGE_API_KEY | 行为 |
|---|---|---|
| **提示词输出 (默认)** | 保留 `your_modelscope_sdk_token` | 生成中文+英文提示词 → 保存到 `图片/提示词/*.txt` |
| **API 直接生成** | 填入真实 ModelScope Token | 调用 ModelScope API 自动生成并下载图片 |

**提示词输出模式使用流程：**
1. 运行 genimg → 提示词文件保存到 `图片/提示词/`
2. 打开提示词 `.txt` 文件，复制内容
3. 粘贴到 [nano banana](https://nanobanana.ai) 或 GPT-Image-2 生成图片
